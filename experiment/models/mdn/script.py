...
parser = argparse.ArgumentParser(
    description="Runs model mdn on the given split.")
...
parser.add_argument('--n_components', type=int, default=100, help='Number of components') 
...
sys.path.append(os.path.join(SCRIPT_DIR, "torch-naut")) # add torch-naut to path

## --------------------------- code from torch-naut/uci/run_mdn.py ---------------------
...
eval_pred = mdn.predict(net, scalers, X_to_predict)

# returns a tensor of [num_datapoints x (num_components x 3)] with the last
# dims corresponding to (num_components x mu, num_components x var, num_components x pi)
def _get_params(p):
...
params_pred = _get_params(torch.tensor(eval_pred, device=device))
